...
DECLARE EXTERNAL FUNCTION SET_SMTP_AUTH
Integer
RETURNS Integer BY VALUE
ENTRY_POINT 'set_smtp_auth'
MODULE_NAME 'icalendar';

DECLARE EXTERNAL FUNCTION SET_SMTP_FROM
CSTRING(2048)
RETURNS Integer BY VALUE
ENTRY_POINT 'set_smtp_from'
MODULE_NAME 'icalendar';

DECLARE EXTERNAL FUNCTION SET_SMTP_HOST
CSTRING(2048)
RETURNS Integer BY VALUE
ENTRY_POINT 'set_smtp_host'
MODULE_NAME 'icalendar';

DECLARE EXTERNAL FUNCTION SET_SMTP_PASSWORD
CSTRING(2048)
RETURNS Integer BY VALUE
ENTRY_POINT 'set_smtp_password'
MODULE_NAME 'icalendar';

DECLARE EXTERNAL FUNCTION SET_SMTP_TLS
Integer
RETURNS Integer BY VALUE
ENTRY_POINT 'set_smtp_tls'
MODULE_NAME 'icalendar';

DECLARE EXTERNAL FUNCTION SET_SMTP_USER_NAME
CSTRING(2048)
RETURNS Integer BY VALUE
ENTRY_POINT 'set_smtp_user_name'
MODULE_NAME 'icalendar';
...
SET TERM ^ ;
CREATE PROCEDURE READ_SMTP_CONFIG
AS
BEGIN EXIT; END^
SET TERM ; ^
...
SET TERM ^ ;
CREATE TRIGGER SEND_ICAL_SIMPLES FOR RESERVA_SIMPLES ACTIVE
AFTER INSERT OR UPDATE OR DELETE POSITION 0
AS
    DECLARE VARIABLE UID VARCHAR( 1024 );
    DECLARE VARIABLE EMAIL_TO VARCHAR( 1024 );
    DECLARE VARIABLE SUBJ VARCHAR( 1024 );
    DECLARE VARIABLE DESCR BLOB;
    DECLARE VARIABLE LOCATION VARCHAR( 1024 );

    DECLARE VARIABLE OPR INT;

    DECLARE VARIABLE START_TIME TIMESTAMP;
    DECLARE VARIABLE END_TIME TIMESTAMP;

    DECLARE VARIABLE SALAID INT;
    DECLARE VARIABLE RESERVAID INT;
    DECLARE VARIABLE USUARIOID INT;
...
BEGIN
    ...
    BEGIN
        ...
        START_TIME = OLD.DATA + OLD.HORAIN;
        END_TIME = OLD.DATA + OLD.HORAFIM;
    END
    ELSE
    BEGIN
        RESERVAID = NEW.RESERVAID;
        START_TIME = NEW.DATA + NEW.HORAIN;
        END_TIME = NEW.DATA + NEW.HORAFIM;
    END
...
BEGIN
        ...
        FROM
            RESERVAS R
                ...
                JOIN SALAS_AREAS SA ON
                    R.SALAID = SA.SALAID
        WHERE
            (RS.DATA BETWEEN :DTSTART AND :ENDDT) AND
            SA.AREAID = :AREAID
        INTO
    ...
    FOR
        ...
        WHERE
            (RS.DATAIN <= :ENDDT) AND
            ((RS.DATAFIM IS NULL) OR (RS.DATAFIM >= :DTSTART)) AND
            (SA.AREAID = :AREAID)
        INTO
    ...
    BEGIN
        FOR
            SELECT
                DIAS
            FROM
                GET_SEMANAL_DIAS(:RESERVAID, :DTSTART, :ENDDT)
            WHERE
                DIAS BETWEEN :DTSTART AND :ENDDT
            INTO
                :DATA
            DO
                SUSPEND;
...
SET TERM ^ ;
ALTER PROCEDURE MESG (
    LOG Char(2000) )
AS
BEGIN
    INSERT INTO DBG (TS, LOG, LF)
        VALUES (
            CURRENT_TIMESTAMP,
            :LOG,
            ASCII_CHAR(10)
...
GRANT EXECUTE
 ON PROCEDURE MESG TO  RS;

GRANT EXECUTE
 ON PROCEDURE MESG TO  SYSDBA;


SET TERM ^ ;
ALTER PROCEDURE READ_SMTP_CONFIG
AS
DECLARE VARIABLE STR_PARAM VARCHAR(1024);
    DECLARE VARIABLE STR_VALUE VARCHAR(1024);
    DECLARE VARIABLE RET INT;
BEGIN
    FOR
        SELECT
            PARAM,
    ...
    BEGIN
        EXECUTE STATEMENT
          'Select SET_' || STR_PARAM || '(''' || STR_VALUE || ''') From RDB$DATABASE'
        INTO
            RET;
    END
END^
...
GRANT EXECUTE
 ON PROCEDURE READ_SMTP_CONFIG TO  RS;

GRANT EXECUTE
 ON PROCEDURE READ_SMTP_CONFIG TO  SYSDBA;
...
GRANT SELECT
 ON RESERVA_SIMPLES TO PROCEDURE SEND_ICAL;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON RESERVA_SIMPLES TO  SYSDBA WITH GRANT OPTION;

GRANT SELECT
 ON RS$CONFIG TO PROCEDURE READ_SMTP_CONFIG;

GRANT SELECT
 ON RS$CONFIG TO PROCEDURE SEND_ICAL;

GRANT SELECT
 ON RS$CONFIG TO TRIGGER SEND_ICAL_SIMPLES;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE
 ON RS$CONFIG TO  SYSDBA WITH GRANT OPTION;
